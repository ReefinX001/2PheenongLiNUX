...
class FrontStoreAdmin {
    ...
    showAlert(message, type = 'success') {
        ...
        // Clear and rebuild notification safely
        wrap.innerHTML = '';
        const strong = document.createElement('strong');
        strong.style.cssText = `display:block; font-size:13px; letter-spacing:.5px; text-transform:uppercase; margin-bottom:4px; color:${type==='success'?'#047857':'#B91C1C'}`;
        strong.textContent = type==='success'?'สำเร็จ':'ผิดพลาด';
    ...
    renderCategories() {
        ...
        this.categories.forEach(category => {
            ...
            editBtn.onclick = () => admin.editCategory(category._id);
            ...
            deleteBtn.onclick = () => admin.deleteCategory(category._id, category.name);
            ...
            content.style.cssText = 'display:flex; gap:20px;';

            // Create icon block
            const iconBlock = document.createElement('div');
            if (category.icon) {
                const img = document.createElement('img');
                img.src = SecurityUtils ? SecurityUtils.sanitizeUrl(category.icon) : category.icon;
                img.alt = SecurityUtils ? SecurityUtils.escapeHtml(category.name) : category.name;
                img.className = 'preview-image';
                img.style.cssText = 'width:64px; height:64px;';
                iconBlock.appendChild(img);
            } else {
                iconBlock.style.cssText = `width:64px;height:64px; border-radius:18px; background:${category.color || '#ccc'}; display:flex; align-items:center; justify-content:center; color:#fff; font-size:20px;`;
                iconBlock.innerHTML = '<i class="fa-solid fa-tag"></i>';
            }
            ...
            if (category.name_en) {
                const engPill = document.createElement('span');
                engPill.className = 'pill';
                engPill.style.background = 'var(--light-gray)';
                engPill.textContent = category.name_en;
                header.appendChild(engPill);
            }

            const statusPill = document.createElement('span');
            statusPill.className = 'pill';
            statusPill.style.cssText = `background:${category.isActive? 'rgba(16,185,129,.12)':'rgba(239,68,68,.12)'}; color:${category.isActive? '#047857':'#B91C1C'};`;
            statusPill.textContent = category.isActive? 'เปิดใช้งาน':'ปิดใช้งาน';
            header.appendChild(statusPill);

            const orderPill = document.createElement('span');
            orderPill.className = 'pill';
            orderPill.style.background = 'var(--soft-blue)';
            orderPill.textContent = `ลำดับ: ${category.order}`;
            header.appendChild(orderPill);

            // Create description
            const desc = document.createElement('div');
            desc.style.cssText = 'font-size:13px; color:var(--text-secondary); line-height:1.5;';
            desc.textContent = category.description || 'ไม่มีคำอธิบาย';

            info.appendChild(header);
            info.appendChild(desc);

            content.appendChild(iconBlock);
