...
exports.getStockSummary = async (req, res) => {
  const io = req.app.get('io');
  try {
    const { start, end, branch_code, branch_id, mode } = req.query;
    if (!start || !end) {
      return res.status(400).json({
        success: false,
    ...
    // Support both branch_code and branch_id parameters
    const branchParam = branch_id || branch_code;
    if (branchParam && branchParam !== 'all') {
      // If it looks like a MongoDB ObjectId, query by branch_id
      if (branchParam.match(/^[0-9a-fA-F]{24}$/)) {
        filter.branch_id = branchParam;
      } else {
        // Otherwise, query by branch_code
        filter.branch_code = branchParam;
      }
    }
