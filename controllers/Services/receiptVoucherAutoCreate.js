...
const Counter = require('../../models/POS/Counter'); // เพิ่ม import Counter
...
class ReceiptVoucherAutoCreate {
  ...
  static async generateDocumentNumberByType(saleType, branchId, session) {
    const date = new Date();
    const yearThai = date.getFullYear() + 543; // ปีพุทธศักราช
    const year = String(yearThai).substr(-2); // เอาเฉพาะ 2 หลักท้าย เช่น 68
    ...
    // สร้าง pattern แบบ RV-680819
    const docPrefix = `${prefix}-${year}${month}${day}`;

    // ใช้ Counter collection สำหรับ running number
    const counterKey = `receipt_voucher_${docPrefix}`;

    const counter = await Counter.findOneAndUpdate(
      {
        key: counterKey,
        reference_value: `${branchId}`
      },
      { $inc: { seq: 1 } },
      {
        new: true,
