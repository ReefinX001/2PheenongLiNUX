...
class InvoicePdfController {
   ...
   */
  static async createInvoicePdf(order) { // Renamed from createQuotationPdf
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.on('end', () => {
            ...
            // Ensure fileName uses invoiceNumber if available, otherwise fallback
            const docNum = order?.invoiceNumber || order?.quotationNumber || order?._id || Date.now();
            const prefix = order?.invoiceNumber ? 'INV' : 'QT'; // Use INV prefix for invoices
            const fileName = `${prefix}-${docNum}.pdf`;
  ...
  return {
   ...
   taxType:           (typeof i.taxType === 'string' ? i.taxType : 'ไม่มี VAT')
        ...
        // —————————————— ใช้ค่าจาก order.summary ที่ Controller เตรียมไว้แล้ว ——————————————
        // order.summary ควรมีรูปแบบ { beforeTax, shipping, discount, tax, netTotal }
        // ถ้าต้องการเก็บตัวแปรแยกเพิ่ม (เช่น amountInWords) ก็ทำได้เช่นเดียวกัน
        ...
        if (order.items.length > 0) {
            ...
            currentY += 5;                  // เว้นบรรทัดเล็กน้อยจากตาราง

            // --- Remove the old SUMMARY_OFFSET block here ---

            // --- Insert new dynamic summary positioning ---
            const rowH        = CONFIG.sizes.textBody * CONFIG.sizes.lineSpacing;
            const padding     = 6;
            const summaryBoxH = rowH * 5 + padding * 2;

            // 1) Height to reserve at bottom (Footer + Terms + Signature)
            ...
            // 2) Bottom limit for summary box
            const bottomAllowedY   = H - margins.bottom - footerTotalH;

            // 3) Compute summaryY, shift up if overflowing
            const EXTRA_OFFSET = 100;               // push summary down by this many points
            let summaryY = currentY + EXTRA_OFFSET;
            if (summaryY + summaryBoxH > bottomAllowedY) {
              summaryY = bottomAllowedY - summaryBoxH;
            }

            // 4) Draw Thai-baht text box on left
            const boxW_th = bodyW * 0.6;
            const boxH_th = ensureHeight(rowH * 1.8);
            const pad_th  = 5;
            const boxY_th = summaryY + summaryBoxH - boxH_th;
            doc.rect(margins.left, boxY_th, boxW_th, boxH_th).fill(CONFIG.color.bgAccent);
            ...
            // 5) Draw numeric summary on right at summaryY
            this._drawSummary(doc, order, margins, bodyW, summaryY);

            // 6) Advance currentY below the summary block
            currentY = summaryY + summaryBoxH + 20;

            // --- คำนวณบล็อก "ลายเซ็น" ---
            // 2) ตำแหน่ง Y ขั้นต่ำของลายเซ็น ไม่ให้ทับ Summary
            const minSigY = currentY + 10;
            // 3) ตำแหน่ง Y ของลายเซ็น จากขอบล่าง ใช้ footerTotalH ที่คำนวณไว้ก่อนหน้า
...
// ใช้โครงสร้าง order.customer: name, taxId, address, phone
    ...
    const quoteFields = [
        ...
        { label: 'การชำระเงิน', labelEn: 'Credit Term', value: order.creditTerm || 'วันครบกำหนด | เงื่อนไขการชำระเงิน ' },
  ...
  static _drawSummary(doc, order, margins, bodyW, startY) {
    // 1. คำนวณตัวเลขใหม่:
    //    • รวมเงิน = ผลรวมของคอลัมน์ amount แต่ละแถว
    //    • ส่วนลด = order.summary.discount
    const sumAmt     = order.items.reduce((acc, i) => acc + (i.amount || 0), 0);
    const totalDisc  = ensureNumberData(order.summary.discount);
    const netAmt     = sumAmt - totalDisc;
    const vatVal     = Math.round(netAmt * 0.07 * 100) / 100;
    const grandTotal = netAmt + vatVal;

    // 2. กำหนด Label ไทยล้วน และค่าตัวเลขให้ตรงลำดับ
    const labels = [
      'รวมเงิน',
      'ส่วนลด',
      'ยอดรวมหลังหักส่วนลด',
    ...
    return startY + (CONFIG.sizes.textBody * CONFIG.sizes.lineSpacing) * 5 + 12;
}
