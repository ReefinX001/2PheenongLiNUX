...
class Receipt_installment {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.on('end', () => {
          ...
          // ใช้ INV prefix สำหรับทั้งสองประเภทเอกสาร (ตามที่ user ต้องการ)
          const prefix = 'INV';
  ...
  static _drawHeader(doc, order, margins, pageW, startY) {
    ...
    // ใช้ documentType ที่ส่งมาจาก order หรือตรวจสอบจาก items
    const isReceiptOnly = order.documentType === 'RECEIPT' ||
                         (!order.documentType && !order.items.some(i =>
                           i.taxType && i.taxType !== 'ไม่มี VAT' && i.taxType !== 'ไม่มีภาษี'
                         ));
    const titleText = isReceiptOnly
  ...
  static _drawCustomerAndInfo(doc, order, margins, bodyW, startY) {
    ...
    const docLabel = isReceiptOnly ? 'เลขที่ใบเสร็จ' : 'ใบกำกับภาษีเลขที่';
    const dateLabel = isReceiptOnly ? 'วันที่ออก' : 'วันที่ออก (ภาษี)';

    // Check if any items have tax
    const anyTax = order.items?.some(i =>
      i.taxType && i.taxType !== 'ไม่มี VAT' && i.taxType !== 'ไม่มีภาษี'
    ) || order.documentType === 'TAX_INVOICE' || order.invoiceType === 'FULL_TAX';

    const receiptFields = [
      {
        label: docLabel,
        labelEn: anyTax ? 'Tax Invoice No.' : 'Receipt No.',
        value: order.receiptNumber || order.taxInvoiceNumber || order.documentNumber || '-'
      },
      {
        label: dateLabel,
        labelEn: 'Issue Date',
  ...
  static _drawItemsTable(doc, order, margins, bodyW, startY, pageH) {
    ...
    // Check if any items have tax (for determining if we show tax column)
    const anyTax = order.items?.some(i =>
      i.taxType && i.taxType !== 'ไม่มี VAT' && i.taxType !== 'ไม่มีภาษี'
    ) || order.documentType === 'TAX_INVOICE' || order.invoiceType === 'FULL_TAX';

    // ใช้ CONFIG.layout.tableCols
    const cols = !isReceiptOnly ? {
