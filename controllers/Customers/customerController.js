...
/**
 * Get customer loans
 * เพิ่มเมธอดสำหรับดึงข้อมูลสินเชื่อของลูกค้า
 */
exports.getCustomerLoans = async (req, res) => {
  try {
    ...
    // Build query for loans/installment orders
    let query = {
      customer: customer._id,
      deleted_at: null
    };
    ...
    const formattedLoans = loans.map(loan => {
      const totalAmount = loan.finalTotalAmount || loan.totalAmount || 0;
      ...
      return {
        ...
        overdueDays: loan.dueDate && loan.dueDate < new Date() ?
    ...
    res.status(500).json({
      success: false,
      message: 'เกิดข้อผิดพลาดในการดึงข้อมูลสินเชื่อของลูกค้า',
      error: error.message
    });
  }
};
 ...
 * เพิ่มเมธอดสำหรับดึงข้อมูลการผ่อนชำระของลูกค้า
 */
exports.getCustomerInstallments = async (req, res) => {
  try {
    ...
    // Import InstallmentPayment model
    ...
    // Get installments with contract information
    const [installments, totalCount] = await Promise.all([
      InstallmentPayment.find(query)
        .populate('contractId', 'contractNumber contractNo totalAmount monthlyPayment status')
        .populate('customerId', 'individual.firstName individual.lastName corporate.companyName')
        .sort({ paymentDate: sortOrder, createdAt: sortOrder })
        .skip(skip)
        .limit(parseInt(limit))
        .lean(),
    ...
    const formattedInstallments = installments.map(installment => ({
      id: installment._id,
      contractNumber: installment.contractId?.contractNumber,
      contractNo: installment.contractId?.contractNo,
      installmentNumber: installment.installmentNumber,
      dueDate: installment.dueDate,
      paymentDate: installment.paymentDate,
      plannedAmount: installment.plannedAmount || 0,
      paidAmount: installment.paidAmount || 0,
      principalAmount: installment.principalAmount || 0,
      ...
      netAmount: installment.netAmount || installment.paidAmount || 0,
      ...
      isLate: installment.dueDate && installment.paymentDate ?
        new Date(installment.paymentDate) > new Date(installment.dueDate) : false,
      daysPastDue: installment.dueDate && installment.paymentDate ?
        Math.max(0, Math.floor((new Date(installment.paymentDate) - new Date(installment.dueDate)) / (1000 * 60 * 60 * 24))) : 0,
      createdAt: installment.createdAt,
      updatedAt: installment.updatedAt
    }));

    // Calculate summary statistics
    const summary = formattedInstallments.reduce((acc, inst) => {
      acc.totalInstallments += 1;
      acc.totalPlanned += inst.plannedAmount;
      acc.totalPaid += inst.paidAmount;
      acc.totalPrincipal += inst.principalAmount;
      acc.totalInterest += inst.interestAmount;
      acc.totalFines += inst.fineAmount;
      acc.totalDiscount += inst.discountAmount;
    ...
    const contracts = await InstallmentOrder.find({
      customer: customer._id,
      deleted_at: null
    }).select('_id contractNumber contractNo').lean();
    ...
    console.error('❌ Error in getCustomerInstallments:', error);
    res.status(500).json({
      success: false,
      message: 'เกิดข้อผิดพลาดในการดึงข้อมูลการผ่อนชำระของลูกค้า',
      error: error.message
    });
  }
};
