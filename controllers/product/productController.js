...
exports.createProduct = async (req, res) => {
  ...
  try {
    ...
    // ตรวจซ้ำ SKU, IMEI
    if (sku) {
      const dupSku = await Product.findOne({ sku, deleted_at: null }).lean();
      if (dupSku) {
        return res.status(400).json({
          success: false,
          error: `Duplicate SKU "${sku}" found in Products.`
        });
      }
    }
    if (imei) {
    ...
    // ------------------------------------------------------------
    // เอา sku ตามที่ลูกค้าส่งมา หรือ '' ถ้าไม่ได้ส่ง
 ...
 * อัปเดตข้อมูลสินค้า + ตรวจซ้ำ (sku/imei)
 */
exports.updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (sku !== undefined) {
      ...
      product.sku = raw.includes('NaN') ? '' : raw;
    }
