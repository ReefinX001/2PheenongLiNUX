...
if (customerType === 'individual') {
  // ตรวจสอบและแก้ไขค่า prefix ที่เป็นค่าว่าง
      ...
      if (customerInfo?.birthDate) {
        ...
        try {
          birthDate = new Date(customerInfo.birthDate);
          // คำนวณอายุจากวันเกิด
          ...
          console.warn('Invalid birth date format:', customerInfo.birthDate);
        }
      } else if (customerInfo?.age) {
  ...
  individual: customerType === 'individual' ? {
    ...custData.individual,
            ...
            birthDate: custData.individual?.birthDate,
            age: custData.individual?.age
  } : undefined,
    ...
    appliedPromotion: appliedPromotions?.find(p => p.productIds?.includes(it.id).lean()) || null
      ...
      await BranchStockHistory.create({
        ...
        customerInfo: customerType === 'individual' ? {
          ...
          birthDate: custData.individual?.birthDate,
          age: custData.individual?.age
        } : undefined, // Use prepared custData with birthDate and age
  ...
  // (5) getPendingReceiptsForVouchers - รายการใบเสร็จที่ยังไม่มีใบสำคัญรับเงิน
  static async getPendingReceiptsForVouchers(req, res) {
    try {
      const { branchCode, limit = 50 } = req.query;

      console.log('🔍 Getting pending receipts for vouchers...', { branchCode, limit });
      ...
      // 1. ดึงรายการขายสดจาก CashSale ที่ยังไม่มีใบสำคัญรับเงิน
      let cashSaleQuery = {
        purchaseType: 'cash',
        paymentMethod: { $in: ['cash', 'transfer', 'card'] }
      };
      ...
      const cashSales = await CashSale.find(cashSaleQuery)
        .populate('customer', 'individual.firstName individual.lastName individual.prefix corporate.companyName')
        .sort('-createdAt')
        .limit(Math.floor(limit * 0.7)) // 70% จาก CashSale
        .lean();
      ...
      for (const sale of cashSales) {
        ...
        if (!existingVoucher) {
          ...
          if (sale.customer) {
            if (sale.customer.individual) {
              const { prefix, firstName, lastName } = sale.customer.individual;
              customerName = `${prefix || ''} ${firstName || ''} ${lastName || ''}`.trim();
            } else if (sale.customer.corporate) {
              customerName = sale.customer.corporate.companyName || 'บริษัท';
            }
          }
          ...
          results.push({
            ...
            totalAmount: sale.subTotal + (sale.vatAmount || 0) - (sale.discount || 0) - (sale.promotionDiscount || 0),
            netAmount: sale.subTotal + (sale.vatAmount || 0) - (sale.discount || 0) - (sale.promotionDiscount || 0),
            source: 'cash_sale',
      ...
      // 2. ดึงรายการจาก BranchStockHistory ที่เป็นการขายแต่ยังไม่มีใบสำคัญรับเงิน
      let historyQuery = {
        change_type: 'OUT',
        reason: { $in: ['ขาย POS', 'ขายสินค้า', 'ขายหน้าร้าน'] },
        total_amount: { $gt: 0 }
      };

      if (branchCode && branchCode !== 'all') {
        historyQuery.branch_code = branchCode;
      }

      const stockHistory = await BranchStockHistory.find(historyQuery)
        .sort('-performed_at')
        .limit(Math.floor(limit * 0.3)) // 30% จาก BranchStockHistory
        .lean();

      for (const history of stockHistory) {
        const existingVoucher = await ReceiptVoucher.findOne({
          'metadata.sourceId': history._id,
          'metadata.sourceType': 'stock_history'
        });

        if (!existingVoucher) {
          // ตรวจสอบว่ามีใน CashSale แล้วหรือไม่ (เพื่อไม่ให้ซ้ำ)
          const existsInCashSale = results.some(item => item.receiptNumber === history.invoice_no);

          if (!existsInCashSale && history.invoice_no) {
            let customerName = 'ลูกค้าทั่วไป';
            if (history.customerInfo) {
              const { prefix, firstName, lastName } = history.customerInfo;
              customerName = `${prefix || ''} ${firstName || ''} ${lastName || ''}`.trim();
            } else if (history.corporateInfo) {
              customerName = history.corporateInfo.companyName || 'บริษัท';
            }

            results.push({
              _id: history._id,
              receiptNumber: history.invoice_no,
              documentNumber: history.invoice_no,
              saleDate: history.sale_date || history.performed_at,
              createdAt: history.performed_at,
              customerName: customerName,
              customer: {
                name: customerName
              },
              totalAmount: history.total_amount || history.net_amount || 0,
              netAmount: history.net_amount || history.total_amount || 0,
              source: 'pos',
      ...
      console.log(`✅ Found ${results.length} pending receipts for voucher creation`);

      res.json({
        success: true,
        message: `พบ ${results.length} รายการใบเสร็จที่ยังไม่มีใบสำคัญรับเงิน`,
        data: results
      });

    } catch (error) {
      console.error('❌ Error getting pending receipts for vouchers:', error);
      res.status(500).json({
        success: false,
  ...
  static async getHistoryReceiptImage(req, res) {
    try {
      ...
      // Debug สำหรับ orderDataForPrint
      console.log('📋 orderDataForPrint Debug:', {
        'orderDataForPrint.staffName': orderDataForPrint.staffName,
        'orderDataForPrint.branchName': orderDataForPrint.branchName,
        'orderDataForPrint.invoiceNo': orderDataForPrint.invoiceNo
      });
